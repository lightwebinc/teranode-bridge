...
type UpTunnel struct {
	// Addrs are host:port targets for this class's ingress on the edge PROXY
	// (8726 subtree, 8727 block by default — the fabric-side lane numbers, see
	// -edge-subtree-port/-edge-block-port), reachable only through the tunnel,
	// tried in order from the last one that worked.
	Addrs []string
	Class string
	Log   *slog.Logger
